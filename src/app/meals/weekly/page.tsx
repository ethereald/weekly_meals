...
export default function WeeklyMealsPage() {
  ...
  // User map for display name lookup
  ...
  function getDisplayName(key: string) {
    // Try username, then id, then fallback to key
  ...
  useEffect(() => {
    async function fetchUsers() {
      ...
      const data = await res.json();
      // Map by username and id for lookup
  ...
  function getCurrentWeekStart(dateStr?: string) {
    ...
    let startDayIdx = weekStart === "Sunday" ? 0
      : weekStart === "Monday" ? 1
      : weekStart === "Tuesday" ? 2
      ...
      : 1;
    // Calculate diff to get start of week
  ...
  // Build grid: 7 days, each cell lists meals for that day
